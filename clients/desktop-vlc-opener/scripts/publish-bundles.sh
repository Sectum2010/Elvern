...
write_package_readme() {
  ...
  cp "${COMMON_README}" "${readme_path}"
  {
    ...
    if [[ "${mode}" == "portable" ]]; then
      ...
      echo "- Install the .NET 8 runtime on the client machine before first use."
    else
...
build_windows_package() {
  ...
  local mode="$3"
  local package_name="elvern-vlc-opener-${VERSION}-${runtime}"
...
if [[ -f "${FRAMEWORK_DLL}" ]]; then
  ...
  if [[ -z "${DOTNET_CMD}" ]]; then
    ...
    if command -v osascript >/dev/null 2>&1; then
      osascript -e 'display alert ".NET 8 Runtime Required" message "Elvern VLC Opener could not find dotnet. Checked /usr/local/share/dotnet/dotnet, /opt/homebrew/share/dotnet/dotnet, /usr/local/bin/dotnet, /opt/homebrew/bin/dotnet, then PATH."' >/dev/null 2>&1 || true
    fi
