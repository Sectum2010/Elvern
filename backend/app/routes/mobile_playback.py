...
def create_mobile_playback_session(
    ...
    assert_user_can_access_media_by_age(request.app.state.settings, user=user, item_id=int(item["id"]), purpose="mobile_playback")
    try:
...
@router.post("/api/mobile-playback/sessions/{session_id}/audio", response_model=MobilePlaybackSessionResponse)
def select_mobile_playback_audio_track(
    session_id: str,
    ...
    try:
        response = _get_mobile_manager(request).select_audio_track(
            session_id,
            user_id=int(user.id),
            auth_session_id=user.session_id,
            username=user.username,
